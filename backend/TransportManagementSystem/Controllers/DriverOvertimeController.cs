...
public class DriverOvertimeController : ControllerBase
{
    ...
    public async Task<ActionResult<List<DriverAvailabilityDto>>> GetDriversAvailability([FromBody] AvailabilityRequestDto request)
    {
        try
        {
    ...
    private async Task<DriverAvailabilityDto> CheckDriverAvailability(Driver driver, DateTime date, double tripDuration, int? excludeTripId)
    {
        ...
        var availability = new DriverAvailabilityDto
        {
            DriverId = driver.Id,
            DriverName = driver.Name,
